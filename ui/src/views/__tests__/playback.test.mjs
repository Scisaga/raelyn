...
test("主导航按星域、故事、简报、播放列表、资料库排列", () => {
  const labels = createNavItems().filter((item) => !item.hidden && !item.system).map((item) => item.label);
  assert.deepEqual(labels, ["星域", "故事", "简报", "播放列表", "资料库"]);
});

test("播放工作台固定按日并且只加载来源记录，不请求事件或简报", async () => {
  ...
  const ctx = {
    ...
    async api(path) {
      calls.push(path);
      if (path.includes("/detail")) return { name: "测试域", earliest_date: "2026-08-01" };
      return {};
    },
  ...
  assert.equal(calls.some((path) => path.includes("/brief")), false);
...
test("播放模板保留记录、播放器、转写和固定按日日历", async () => {
  ...
  assert.doesNotMatch(template, /事件审核|周期总结|播放列表设置/);
});
