...
import { createPlaylistEventMapMethods } from "./event-map-model.js";
...
export function createPlaylistViewMethods() {
  return {
    async loadPlaylistPage() {
      ...
      if (pid && this._playlistEventMapPlaylistId && String(this._playlistEventMapPlaylistId) !== pid) {
        this.playlistEventMapReset();
      }
      if (pid) this._playlistEventMapPlaylistId = pid;
    ...
    async playlistSetSubview(nextSubview) {
      ...
      if (this.playlistSubview === "main") {
        ...
        const [, manifest] = await Promise.all([
          ...
          this.playlistEventMapLoadManifest({ silent: true }).catch(() => null),
        ]);
        if (manifest && this.playlistEventMapPollIsActive(manifest)) {
          this.playlistEventMapSchedulePoll();
        } else this.playlistEventMapStopPolling();
      }
      this._syncUrl({ push: false });
    },
    ...
    async playlistSubmitEventExtraction({ force = false } = {}) {
      ...
      if (!pid || this.playlistEventMapBackfillSubmitting) return;
      const activeJob = this.playlistEventMapActiveBackfillJob();
      const activeForce = Boolean(activeJob?.force || activeJob?.payload?.force);
      if ((!shouldForce && activeJob) || (shouldForce && activeForce)) return;
      try {
        this.playlistEventMapBackfillSubmitting = true;
        ...
        this.playlistEventMapBackfillJob = result && result.backfill_job ? result.backfill_job : result || null;
        ...
        if (this.playlistEventMapActiveBackfillJob()) this.playlistEventMapSchedulePoll();
      } catch (e) {
        const msg = e && e.message ? e.message : String(e);
        this.playlistEventsError = msg;
        this.playlistEventMapError = msg;
        this.globalStatus = `error: ${msg}`;
      } finally {
        this.playlistEventMapBackfillSubmitting = false;
    ...
    async playlistSettingsForceExtractEvents() {
      const ok =
        ...
        window.confirm(
          "全部重新抽取会先停止当前播放列表相关的事件抽取、事件 embedding 与事件地图构建任务，然后重新抽取所有已有 plain transcript 的视频事件。任务历史会保留。确认继续？"
        );
      if (!ok) return;
