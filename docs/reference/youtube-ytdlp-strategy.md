...
YouTube 媒体传输阶段若出现代理 `CONNECT ... 502`、`connection closed`、`connection reset`、媒体 URL `HTTP 502`，或原视频页解析得到的 `formats` 为空，下载不会只对已经失败的结果长时间盲重试。每轮解析内由 yt-dlp 对同一媒体 URL 执行 `retries=1`，即首次请求失败后只短重试一次；原 selector 最多从视频页解析两轮（首次解析加一次重新解析）。每轮使用独立临时产物，不复用上一轮 `.part` 文件，并保持相同的 cookies、`YTDLP_PROXY` 与 impersonation。`formats` 为空只重新解析原页面，不切 selector；媒体传输错误在原 selector 两轮都失败后，最多让一个既有 fallback selector 再解析一轮，使 yt-dlp 有机会取得另一组媒体连接，但不会无限切换格式。重新解析仍可能返回相同的 formats 或 CDN 节点，因此不承诺一定换服务器。

上述恢复路径耗尽后会抛出 `youtube_media_transport` 或 `youtube_no_video_formats` 瞬时错误。worker 不在进程内等待，而是释放下载槽位，将任务重新设为 `pending` 并把下一次执行时间写入 `job.scheduled_for`；最多执行 4 次，前三次延迟基准为 2 分钟、10 分钟、30 分钟，并按 job id 与 attempt 加确定性 `±20%` jitter，避免同批失败任务同时苏醒。任务事件会记录原因码、`scheduled_for`、backoff 和熔断状态；第 4 次仍失败才进入 `failed`。普通确定性下载错误不使用这一扩展窗口，仍按原有最多 2 次处理。
