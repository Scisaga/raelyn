...
def install_if_needed() -> None:
    ...
    Control:
      - Set RAELYN_LOG_TIMESTAMP=1 to force enable.
      - Set RAELYN_LOG_TIMESTAMP=0 to force disable.
      - Back-compat: also accepts VIDEOSYNC_LOG_TIMESTAMP.
    """
    flag = (os.getenv("RAELYN_LOG_TIMESTAMP") or os.getenv("VIDEOSYNC_LOG_TIMESTAMP") or "").strip()
    if flag == "0":
        return
    force = flag == "1"
