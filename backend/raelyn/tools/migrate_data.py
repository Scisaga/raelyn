...
def _migrate_schema(conn) -> None:
    """
    复制 runtime migration 中目标库需要的结构变更，避免导入会绑定 .env 的 raelyn.db。

    数据迁移会在 COPY 前准备并清空目标表；旧源库的 story identity 首次关联仍由
    runtime migration 负责，因此这里不复制 db.py 中的 identity seed/relink 步骤。
    """
    insp = inspect(conn)
